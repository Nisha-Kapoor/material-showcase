...
import com.google.mlkit.md.productsearch.RecommendationAdapter
...
class StaticObjectDetectionActivity : AppCompatActivity(), View.OnClickListener {
    ...
    private var effectsButton: ExtendedFloatingActionButton? = null
    private var bottomSheetBehavior: BottomSheetBehavior<View>? = null
    private var bottomSheetTitleView: TextView? = null
    private var recRecyclerView: RecyclerView? = null
    ...
    override fun onClick(view: View) {
        when (view.id) {
            R.id.effects_search_button -> {
                effectsButton?.isEnabled = false
...
//                bottomSheetBehavior?.state = BottomSheetBehavior.STATE_COLLAPSED
                val view = layoutInflater.inflate(R.layout.effects_bottom_sheet, null)
    ...
    private fun showSearchResults(searchedObject: SearchedObject) {
        ...
        val recList = searchedObject.recommendationtList
        recRecyclerView?.adapter = RecommendationAdapter(recList)
    ...
    private fun setUpBottomSheet() {
        ...
        effectsButton = findViewById<ExtendedFloatingActionButton>(R.id.effects_search_button).apply {
            setOnClickListener(this@StaticObjectDetectionActivity)
        }
        bottomSheetTitleView = findViewById(R.id.recommendations_bottom_sheet_title)
        recRecyclerView = findViewById<RecyclerView>(R.id.rec_recycler_view).apply {
            setHasFixedSize(true)
            layoutManager = LinearLayoutManager(this@StaticObjectDetectionActivity)
            adapter = RecommendationAdapter(ImmutableList.of())
        }
    }
    ...
    private fun onSearchCompleted(detectedObject: DetectedObjectInfo, recommendationList: List<String>) {
        Log.d(TAG, "Search completed for object index: ${detectedObject.objectIndex}")
        searchedObjectMap[detectedObject.objectIndex] = SearchedObject(resources, detectedObject, recommendationList)
